...
class MITBIHLoader(Dataset):
    def __init__(self, args, root_path, flag=None):
        ...
        self.X, self.y = self.load_mitbih(self.data_path, self.label_path, flag=flag)
    ...
    def load_train_val_test_list(self, label_path, a=0.6, b=0.8):
        ...
        others_list = list(
            data_list[np.where(data_list[:, 0] == 5)][:, 1]
        )  # Others IDs

        train_ids = (
            norm_list[: int(a * len(norm_list))]
            + pvc_list[: int(a * len(pvc_list))]
            + pac_list[: int(a * len(pac_list))]
            + fusion_list[: int(a * len(fusion_list))]
            + paced_list[: int(a * len(paced_list))]
            + others_list[: int(a * len(others_list))]
        )
        val_ids = (
            norm_list[int(a * len(norm_list)) : int(b * len(norm_list))]
            + pvc_list[int(a * len(pvc_list)) : int(b * len(pvc_list))]
            + pac_list[int(a * len(pac_list)) : int(b * len(pac_list))]
            + fusion_list[int(a * len(fusion_list)) : int(b * len(fusion_list))]
            + paced_list[int(a * len(paced_list)) : int(b * len(paced_list))]
            + others_list[int(a * len(others_list)) : int(b * len(others_list))]
        )
        test_ids = (
            norm_list[int(b * len(norm_list)) :]
            + pvc_list[int(b * len(pvc_list)) :]
            + pac_list[int(b * len(pac_list)) :]
            + fusion_list[int(b * len(fusion_list)) :]
            + paced_list[int(b * len(paced_list)) :]
            + others_list[int(b * len(others_list)) :]
        )

        return train_ids, val_ids, test_ids

    def load_mitbih(self, data_path, label_path, flag=None):
        """
        Loads mit-bih data from npy files in data_path based on flag and ids in label_path
        Args:
            data_path: directory of data files
            label_path: directory of label.npy file
            flag: 'train', 'val', or 'test'
