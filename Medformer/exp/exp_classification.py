...
class Exp_Classification(Exp_Basic):
    ...
    def _build_model(self):
        ...
        self.args.num_class = int(np.max(test_data.y)) + 1  # redefine num_class
        # model init
        model = (
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        try:
            ...
            present_classes = np.unique(trues)
            if len(present_classes) < self.args.num_class:
                # Compute AUROC only for present classes
                mask = np.isin(np.arange(self.args.num_class), present_classes)
                probs_filtered = probs[:, mask]
                trues_onehot_filtered = trues_onehot[:, mask]
                auroc = roc_auc_score(trues_onehot_filtered, probs_filtered, multi_class="ovr")
            else:
                auroc = float('nan')
